...
contract OneDoge is Context, IERC20, Ownable {
    ...
    constructor(address router) public {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router); // Uniswap Router For Ethereum
    ...
    function _transfer(
        ...
        if(sellCooldownEnabled && recipient == uniswapV2Pair && !isTaxless[sender]) {
            require(sellCooldown[sender] < block.timestamp, "Err: Sell Cooldown");
            if(lastSellCycleStart[sender] + sellCooldownTime < block.timestamp){
