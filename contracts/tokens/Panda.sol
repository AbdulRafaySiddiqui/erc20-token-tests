...
contract Panda is Context, IERC20, Ownable {
    ...
    uint256 public maxTxAmount = _tokenTotal.mul(5).div(10000);// 0.05%
    uint256 public minTokensBeforeSwap = 100e18;
    uint256 public sellAmountLimit = 10000;// 100%
    ...
    constructor(address router) public {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router); // Pancakeswap Router
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
        uniswapV2Router = _uniswapV2Router;
       
        buybackWallet = address(new Wallet());
        
        // address _owner = 0xfed589bb82D3cAE51dfC5Ffed4a9A50627df300C;
        // teamWallet = 0xfed589bb82D3cAE51dfC5Ffed4a9A50627df300C;
        // admin = 0xfed589bb82D3cAE51dfC5Ffed4a9A50627df300C;
    ...
    function _transfer(
        ...
        require(listedAt != 0 || isTaxless[sender] || isTaxless[recipient], "Not Listed yet!");
        
        if(!isTaxless[sender] && !isTaxless[recipient] && listedAt + 3 minutes >= block.timestamp) {
            // don't allow to buy more than 0.01% of total supply for 3 minutes after launch
            require(sender != uniswapV2Pair || balanceOf(recipient).add(amount) <= _tokenTotal.div(10000),"AntiBot: Buy Banned!");
        ...
        if(buybackEnabled && lastbuyback + buybackInterval < block.timestamp) {
    ...
    function calculateCooldownAmountLimit(address sender) internal view returns (uint256) {
        ...
        uint256 limitMultiplier = (block.timestamp - listedAt) / cooldownTime; // this calculates how many days since listed
    ...
    function collectFee(address account, address to, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(buybackFee != 0){
            ...
            _reflectionBalance[address(this)] = _reflectionBalance[address(this)].add(_buybackFee.mul(rate));
            if (_isExcluded[address(this)]) {
                _tokenBalance[address(this)] = _tokenBalance[address(this)].add(_buybackFee);
            }
            feeTotal = feeTotal.add(_buybackFee);
            emit Transfer(account,address(this),_buybackFee);
        }
    ...
    function setBuyback(uint _buybackInterval, bool _buybackEnabled) external onlyOwnerAndAdmin {
